...
void processCommands(Book bookM[], int iBookCount, char *pszCommandFileName);
...
{
    Book      bookM[MAX_BOOKS];             // array of book items
    int       iBookCnt;                     // number of items in bookM;
    ...
    iBookCnt = getBooks(bookM, pszBookFileName);
    printBooks("Initial Books", bookM, iBookCnt);
    
    sortBooks(bookM, iBookCnt);
    printBooks("Sorted Books", bookM, iBookCnt);
    printf("\n");

    // process the command input file
    processCommands(bookM, iBookCnt, pszCommandFileName);

    // print the book after processing the commandfile
    printBooks("Resulting Books", bookM, iBookCnt);
    printf("\n");
    return 0;
}

/******************** processCommands **************************************
    void processCommands(Book bookM[], int iBookCnt
        , char *pszCommandFileName)
Purpose:
    Reads the Comand file to process commands.  There are two major
    types of commands:  CUSTOMER and BOOK.
Parameters:
    I/O Book  bookM[]                 array of book
    ...
    */
    while (fgets(szInputBuffer, 100, pfileCommand) != NULL)
    {
        ...
        {   // CUSTOMER command
            ...
            if (bGotToken)
                processCustomerCommand(bookM, iBookCnt, szSubCommand
        ...
        {   // BOOK command
            bGotToken = getSimpleToken(szInputBuffer, &iBufferPosition, szSubCommand);
            if (bGotToken)
                processBookCommand(bookM, iBookCnt, szSubCommand
