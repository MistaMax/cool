...
{
	int i = 0, j = 1; 
	queueElementT tmp, tmp2;
	for (i = 0; i < queue->tail / 2; i++) 
	{
		tmp = queue->elements[queue->head + i]; //starts from beginning and saves it
		tmp2 = queue->elements[queue->tail - j]; //saves end of queue
		queue->elements[queue->head + i] = tmp2; //inputs to incremented point/beginning
		queue->elements[queue->tail - j] = tmp; // inputs de-incremented point/end
		j++;
	}
}
